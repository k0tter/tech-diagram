...
すべて**無修正の生成結果**です。作例3点はギャラリー用の作例、テンプレート10点は
[templates/](templates/) の同梱スペックから再生成できます(クリックで原寸)。
...
- **プレビュー**: `--emit-png` で実描画 PNG(draw.io CLI 自動検出)、`--emit-svg` で
  ...
  複数タブは `<出力名>.<01からの連番>-<安全化したタブ名>.*` で出力し、似た名前でも上書きしない
...
- **誤検知ゼロの校正**: 同梱テンプレート10件+リファレンススペック5件の
  再現可能な掃引で、W16〜W21 の検出 0 件
...
[evals/](evals/) の再現可能な評価スイートで、自然言語の依頼から人手修正なしの
0 エラー・0 警告 `.drawio` を作れるかを検証します。機械判定できる要件は
`evals/check_diagram.py`、判断・報告要件は別の新品コンテキストのエージェントが
採点します。公開するベンチマーク値には raw run artifact と client / model /
version / environment が必要なため、監査できない過去の集計値はここでは掲示しません。
手順と fixture は [evals/README.md](evals/README.md) を参照してください。
